...
function getAdminJS() {
  return [
    ...
    "  mc.innerHTML='<div class=\'stats-grid\'>'+",
    "    '<div class=\'stat-card\'><div class=\'stat-label\'><i class=\'ti ti-building\'></i> Propiedades activas</div><div class=\'stat-val\'>'+activas+'</div><div class=\'stat-sub up\'>Total: '+props.length+'</div></div>'+",
    "    '<div class=\'stat-card\'><div class=\'stat-label\'><i class=\'ti ti-check\'></i> Vendidas</div><div class=\'stat-val\'>'+vendidas+'</div></div>'+",
    "    '<div class=\'stat-card\'><div class=\'stat-label\'><i class=\'ti ti-users\'></i> Leads</div><div class=\'stat-val\' id=\'statLeadsNum\'>-</div></div>'+",
    "    '</div>'+",
    "    '<div class=\'table-card\'>'+",
    "    '<div class=\'table-header\'><div class=\'card-title\'>Propiedades recientes</div><button class=\'btn btn-primary btn-sm\' onclick=\'openModal(null)\'><i class=\'ti ti-plus\'></i> Nueva</button></div>'+",
    ...
    "  var mc=document.getElementById('mainContent');if(!mc)return;",
    "  mc.innerHTML='<div class=\'import-panel\' id=\'importPanel\'><div style=\'font-size:15px;font-weight:700;color:var(--navy);margin-bottom:6px\'>Importar CSV</div><div style=\'font-size:13px;color:var(--text3);margin-bottom:16px\'>Sube el archivo propiedades.csv exportado de Wix.</div><div class=\'drop-zone\' id=\'dropZone\' onclick=\'triggerFileInput()\'><div style=\'font-size:13px;color:var(--text3)\'><strong>Click para seleccionar</strong> o arrastra el CSV aquí</div><input type=\'file\' id=\'csvFile\' accept=\'.csv\' style=\'display:none\' onchange=\'handleFile(this.files[0]);\'></div><div class=\'import-preview\' id=\'importPreview\'><div style=\'display:flex;justify-content:space-between;background:#f0f9ff;border:1px solid #bae6fd;border-radius:6px;padding:12px 16px;margin:14px 0\'><span id=\'previewText\'>0 propiedades</span><span id=\'previewFile\' style=\'color:var(--text3)\'></span></div><div class=\'progress-wrap\' id=\'progressWrap\'><div class=\'progress-bar-bg\'><div class=\'progress-bar\' id=\'progressBar\'></div></div><div class=\'progress-label\' id=\'progressLabel\'></div></div><div class=\'import-actions\'><button class=\'btn btn-ghost\' onclick=\'cancelImport()\'>Cancelar</button><button class=\'btn btn-navy\' id=\'btnImport\' onclick=\'doImport()\'>Importar al KV</button></div></div></div>'+",
    "    '<div class=\'table-card\'>'+",
    "    '<div class=\'table-header\'><div class=\'search-box\'><i class=\'ti ti-search\'></i><input type=\'text\' id=\'searchInput\' placeholder=\'Buscar propiedad...\' oninput=\'renderTable()\'></div>'+",
    "    '<div style=\'display:flex;gap:8px\'><button class=\'btn btn-ghost btn-sm\' onclick=\'toggleImport()\'><i class=\'ti ti-upload\'></i> CSV</button><button class=\'btn btn-primary btn-sm\' onclick=\'openModal(null)\'><i class=\'ti ti-plus\'></i> Nueva</button></div></div>'+",
    "    '<div id=\'tableWrap\'></div></div>';",
    "  renderTable();",
    "}",
    "function renderLeadsPage(){",
    "  var mc=document.getElementById('mainContent');if(!mc)return;",
    "  mc.innerHTML='<div id=\'leadsContent\' style=\'font-size:13px;color:var(--text3)\'>Cargando leads...</div>';",
    "  showLeads();",
    "}",
    ...
    "  if(!data||!data.length){el.innerHTML='<div style=\'padding:32px;text-align:center;color:var(--text3)\'>Sin propiedades</div>';return;}",
    "  var html='<table><thead><tr><th>Propiedad</th><th>Tipo</th><th>Precio</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>';",
    "  data.forEach(function(p){",
    "    var badge=p.estado==='Vendida'?'badge-red':p.estado==='Pausada'?'badge-gray':'badge-green';",
    "    html+='<tr><td><div class=\'prop-name\'>'+escHtml(p.titulo||'')+'</div><div class=\'prop-loc\'>'+escHtml(p.municipio||p.zona||'')+'</div></td>'+",
    "      '<td>'+escHtml(p.tipo||'')+'</td>'+",
    "      '<td class=\'price-cell\'>'+escHtml(p.priceFormatted||p.precio||'')+'</td>'+",
    "      '<td><span class=\'badge '+badge+'\'>'+escHtml(p.estado||'Activa')+'</span></td>'+",
    "      '<td><div class=\'actions-cell\'><div class=\'icon-btn\' title=\'Editar\' data-id=\''+idx+'\' onclick=\'var d=props[this.dataset.id];if(d)openModal(d.id||d.slug)\'><i class=\'ti ti-edit\'></i></div>'+",
    "      '<div class=\'icon-btn\' title=\'Eliminar\' data-id=\''+idx+'\' onclick=\'var d=props[this.dataset.id];if(d)deleteProp(d.id||d.slug,d.titulo||\'\')\'  style=\'color:var(--red)\'><i class=\'ti ti-trash\'></i></div></div></td></tr>';",
    "  });",
    "  html+='</tbody></table>';",
    "  el.innerHTML=html;",
    "}",

...
function getAdminJS() {
  return [
    ...
    "  setEl('statActivas',props.filter(function(p){return p.estado==='Activa';}).length);",
    "  setEl('statVenta',props.filter(function(p){return p.operacion==='Venta'||p.operacion==='Venta/Renta';}).length);",
    "  setEl('statRenta',props.filter(function(p){return p.operacion==='Renta'||p.operacion==='Venta/Renta';}).length);",
    "}",
    "function renderTable(){var tw=document.getElementById('tableWrap');if(!tw){setTimeout(renderTable,100);return;}",
    "  var si=document.getElementById('searchInput');var q=si?si.value.toLowerCase():'';",
    "  var filtered=q?props.filter(function(p){return(p.titulo||'').toLowerCase().includes(q)||(p.zona||'').toLowerCase().includes(q)||(p.tipo||'').toLowerCase().includes(q);}):props.slice();",
    "  renderTableInto(tw,filtered);return;",
    "  if(!filtered.length){tw.innerHTML='<div style=\"text-align:center;padding:48px;color:#6c757d\">No hay propiedades.</div>';return;}",
    ...
    "  if(!el)return;",
    "  if(!data||!data.length){el.innerHTML='<p style=\"padding:32px;text-align:center;color:#8A9BB0\">Sin propiedades</p>';return;}",
    "  var rows='';",
    "  for(var i=0;i<data.length;i++){",
    "    var p=data[i];",
    "    var est=p.estado||'Activa';",
    "    var estColor=est==='Vendida'?'#fee2e2;color:#991b1b':est==='Pausada'?'#f1f5f9;color:#475569':'#dcfce7;color:#166534';",
    "    var pid=encodeURIComponent(p.id||p.slug||i);",
    "    rows+='<tr>';",
    "    rows+='<td style=\"padding:12px 16px;border-bottom:1px solid #f0f0f0\">';",
    "    rows+='<div style=\"font-weight:600;color:#0D1B3E\">'+escHtml(p.titulo||'')+'</div>';",
    "    rows+='<div style=\"font-size:11px;color:#8A9BB0\">'+escHtml(p.municipio||p.zona||'')+'</div>';",
    "    rows+='</td>';",
    "    rows+='<td style=\"padding:12px 16px;border-bottom:1px solid #f0f0f0;font-size:13px\">'+escHtml(p.tipo||'')+'</td>';",
    "    rows+='<td style=\"padding:12px 16px;border-bottom:1px solid #f0f0f0;font-weight:700;color:#F5820D\">'+escHtml(p.priceFormatted||p.precio||'')+'</td>';",
    "    rows+='<td style=\"padding:12px 16px;border-bottom:1px solid #f0f0f0\"><span style=\"padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;background:'+estColor+'\">'+escHtml(est)+'</span></td>';",
    "    rows+='<td style=\"padding:12px 16px;border-bottom:1px solid #f0f0f0\">';",
    "    rows+='<button onclick=\"openModal(decodeURIComponent(this.dataset.id))\" data-id=\"'+pid+'\" style=\"margin-right:4px;padding:4px 10px;border:1px solid #E2E8F0;border-radius:5px;background:#fff;cursor:pointer;font-size:12px\">Editar</button>';",
    "    rows+='<button onclick=\"var d=props.find(function(x){return (x.id||x.slug)===decodeURIComponent(this.dataset.id);}.bind(this));if(d)deleteProp(d.id||d.slug,d.titulo||String())\" data-id=\"'+pid+'\" style=\"padding:4px 10px;border:1px solid #fca5a5;border-radius:5px;background:#fff;color:#ef4444;cursor:pointer;font-size:12px\">Eliminar</button>';",
    "    rows+='</td></tr>';",
    "  }",
    "  el.innerHTML='<table style=\"width:100%;border-collapse:collapse\"><thead><tr>'+",
    "    '<th style=\"padding:10px 16px;font-size:11px;font-weight:700;text-transform:uppercase;color:#8A9BB0;background:#F8FAFC;text-align:left\">Propiedad</th>'+",
    "    '<th style=\"padding:10px 16px;font-size:11px;font-weight:700;text-transform:uppercase;color:#8A9BB0;background:#F8FAFC;text-align:left\">Tipo</th>'+",
    "    '<th style=\"padding:10px 16px;font-size:11px;font-weight:700;text-transform:uppercase;color:#8A9BB0;background:#F8FAFC;text-align:left\">Precio</th>'+",
    "    '<th style=\"padding:10px 16px;font-size:11px;font-weight:700;text-transform:uppercase;color:#8A9BB0;background:#F8FAFC;text-align:left\">Estado</th>'+",
    "    '<th style=\"padding:10px 16px;font-size:11px;font-weight:700;text-transform:uppercase;color:#8A9BB0;background:#F8FAFC;text-align:left\">Acciones</th>'+",
...
/* TABLE */
.table-card{background:var(--wh);border:1px solid var(--border);border-radius:10px;overflow:visible;box-shadow:var(--shadow)}

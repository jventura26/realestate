...
const body=`<div class="detail-container"><div style="margin-bottom:32px"><div class="breadcrumb"><a href="/">Home</a> / <a href="/propiedades.html?tipo=${encodeURIComponent(prop.tipo)}">${escapeHtml(prop.tipo)}s</a> / <span>${escapeHtml(prop.title)}</span></div></div><div class="detail-gallery"><div class="gallery-main"><img id="mainImg" src="${escapeHtml(prop.mainImageThumb||'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=800&q=70')}" alt="${escapeHtml(prop.tipo||'Propiedad')} ${escapeHtml(prop.title)} en ${escapeHtml(prop.locationFull)} - INMUHUB.COM" loading="eager" width="800" height="500"></div>${galleryHTML}</div><div class="detail-content"><h1>${escapeHtml(prop.title)}</h1><div class="detail-price">${escapeHtml(prop.priceFormatted)}</div><div class="specs-grid">${specHTML}</div>${descHTML}<div class="info-card">${infoHTML}</div>` + `<section style="padding:48px 6%;background:#f9fafb;border-top:1px solid #e5e7eb"><div style="max-width:1200px;margin:0 auto"><h3 style="font-size:28px;font-weight:700;color:#1a2a4e;margin-bottom:40px;text-align:center">Calculadora Hipotecaria</h3><div style="background:white;border-radius:16px;padding:40px;box-shadow:0 4px 20px rgba(0,0,0,0.08)"><div style="margin-bottom:32px;padding:24px;background:#e3f2fd;border-radius:12px"><label style="display:block;font-weight:600;color:#1a2a4e;margin-bottom:12px">Precio: Q <span id="priceDisplay">${(parseFloat(prop.priceNumeric||0)).toLocaleString('es-GT')}</span></label><input type="number" id="price" value="${Math.round(parseFloat(prop.priceNumeric||0))}" style="width:100%;padding:12px;border:1px solid #d1d5db;border-radius:8px"></div><div style="margin-bottom:32px;padding:24px;background:#fff3e0;border-radius:12px"><label style="display:block;font-weight:600;color:#1a2a4e;margin-bottom:12px">Cuota Inicial (20%): Q <span id="downPaymentDisplay">${(Math.round((parseFloat(prop.priceNumeric||0))*0.2)).toLocaleString('es-GT')}</span></label><input type="number" id="downPayment" value="${Math.round((parseFloat(prop.priceNumeric||0))*0.2)}" style="width:100%;padding:12px;border:1px solid #d1d5db;border-radius:8px"></div><div style="margin-bottom:32px"><label style="display:block;font-weight:600;color:#1a2a4e;margin-bottom:12px">Plazo: <span id="termDisplay">20</span> años</label><input type="range" id="term" min="5" max="30" value="20" style="width:100%;height:8px;background:#e5e7eb;border-radius:4px"></div><div style="margin-bottom:40px"><label style="display:block;font-weight:600;color:#1a2a4e;margin-bottom:12px">Tasa: <span id="rateDisplay">7.5</span>%</label><input type="range" id="rate" min="3" max="12" step="0.1" value="7.5" style="width:100%;height:8px;background:#e5e7eb;border-radius:4px"></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:40px"><div style="background:linear-gradient(135deg,#ffa500 0%,#ff8c00 100%);border-radius:12px;padding:32px;color:white"><p style="font-size:12px;opacity:0.9;margin:0 0 8px 0">Cuota Inicial</p><p style="font-size:32px;font-weight:700;margin:0" id="summaryDown">Q ${(Math.round((parseFloat(prop.priceNumeric||0))*0.2)).toLocaleString('es-GT')}</p></div><div style="background:linear-gradient(135deg,#1a2a4e 0%,#2d4069 100%);border-radius:12px;padding:32px;color:white"><p style="font-size:12px;opacity:0.8;margin:0 0 8px 0">Cuota Mensual</p><p style="font-size:32px;font-weight:700;color:#ffa500;margin:0" id="monthlyPayment">Q 0</p></div></div><div style="text-align:center"><a href="https://wa.me/502XXXXXXXX" target="_blank" style="display:inline-block;background:#1a2a4e;color:white;padding:16px 48px;border-radius:8px;font-weight:600;text-decoration:none">Contactar Asesor →</a></div></div></div></section><script>const price=document.getElementById('price');const down=document.getElementById('downPayment');const term=document.getElementById('term');const rate=document.getElementById('rate');function fmt(n){return n.toLocaleString('es-GT')}function calc(){const propPrice=parseFloat(price.value)||0;const downPay=parseFloat(down.value);const principal=propPrice-downPay;const r=parseFloat(rate.value)/100/12;const n=parseFloat(term.value)*12;const monthly=principal>0?(principal*r*Math.pow(1+r,n))/(Math.pow(1+r,n)-1):0;document.getElementById('priceDisplay').textContent=fmt(Math.round(propPrice));document.getElementById('downPaymentDisplay').textContent=fmt(Math.round(downPay));document.getElementById('summaryDown').textContent='Q '+fmt(Math.round(downPay));document.getElementById('monthlyPayment').textContent='Q '+fmt(Math.round(monthly));}price.addEventListener('input',calc);down.addEventListener('input',calc);term.addEventListener('input',calc);rate.addEventListener('input',calc);calc();</script>` + `<section style="padding:48px 6%;background:white">

...
<section id="contacto" style="background:var(--ink2);position:relative;overflow:hidden">
  ...
  return layout({ title: null, desc: `Propiedades premium en Guatemala. ${props.length} propiedades disponibles. Asesoría personalizada.`, canonical: '/', body, scripts: '<script src="https://zona-inmu.tours-virtuales-gt.workers.dev/dynamic-grid.js"></script>' });
}
...
<div id="g" class="prop-grid" style="background:var(--ink);min-height:400px">
  ...
  return layout({ title: 'Catálogo Propiedades Premium Guatemala', desc: `${props.length} propiedades premium en Guatemala. Casas, apartamentos, fincas e inversiones con asesoría.`, canonical: '/propiedades.html', body, scripts: filterJS + '<script src="https://zona-inmu.tours-virtuales-gt.workers.dev/dynamic-grid.js"></script>' });
}
...
<style>@media(max-width:768px){#mobileWa{display:block!important;margin-top:24px}}</style>`;
  ...
  return layout({ title: prop.title, desc: metaDesc, canonical: `/propiedades/${prop.slug}.html`, ogImage: prop.mainImage, body, scripts: '<script src="https://zona-inmu.tours-virtuales-gt.workers.dev/dynamic-grid.js"></script>' });
}
